...
var MaxCreeps = CreepMax['harvester'] + CreepMax['builder'] + CreepMax['upgrader'];
...
//These need to be adjusted at the start of a room to WORK,CARRY,MOVE
    if (TotalCreeps < MaxCreeps)
    {
        if(harvesters.length < MaxHarv) {
            var newName = Game.spawns['Spawn1'].createCreep(WorkerBody1, undefined, {role: 'harvester', roleOriginal: 'harvester'});
            console.log('Spawning new harvester: ' + newName);
        }
        else {
            if(upgraders.length < MaxUpgrade) {
                var newName = Game.spawns['Spawn1'].createCreep(WorkerBody1, undefined, {role: 'upgrader', roleOriginal: 'upgrader'});
                console.log('Spawning new upgrader: ' + newName);
            }
            else {
                if(builders.length < MaxBuild) {
                    var newName = Game.spawns['Spawn1'].createCreep(WorkerBody1, undefined, {role: 'builder', roleOriginal: 'builder'});
